...
import { isReelAudioKitSoundId, reelAudioKit, reelAudioKitSounds } from "./audio-kit";
...
describe("reel audio kit", () => {
  it("ships every catalogued sound with the exact bytes the catalog claims", async () => {
    for (const sound of reelAudioKit) {
  ...
  it("only ships rights-cleared sounds", () => {
  ...
  it("indexes and narrows sound IDs", () => {
    expect(Object.keys(reelAudioKitSounds)).toHaveLength(reelAudioKit.length);
